...
function buildCommand (textEditor, strictMode) {
  ...
  makensis.on('close', (code) => {
    if (code === 0) {
      if (hasWarning === true) {
        if (config.showNotifications) vscode.window.showWarningMessage("Compiled with warnings");
        console.warn(stdErr);
      } else {
        if (config.showNotifications) vscode.window.showInformationMessage("Compiled successfully");
      }
    } else {
