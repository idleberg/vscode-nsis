...
export async function compilerOutput(
	data: Makensis.CompilerData,
): Promise<void> {
	if (!data?.line) {
		return;
	}
...
export async function compilerExit(
	data: Makensis.CompilerOutput,
): Promise<void> {
	...
	if (data["status"] === 0) {
		...
		const openButton =
			(await isWindowsCompatible()) === true &&
			data.outFile?.length &&
			outfileExists
				? "Run"
				: "";
		const revealButton =
			outfileExists && ["win32", "darwin", "linux"].includes(platform())
				? "Reveal"
				: "";
...
export async function flagsCallback(
	data: Makensis.CompilerOutput,
): Promise<void> {
	infoChannel.clear();
...
export async function versionCallback(
	data: Makensis.CompilerOutput,
): Promise<void> {
	infoChannel.clear();
