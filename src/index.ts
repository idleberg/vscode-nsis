...
export async function activate(context: ExtensionContext): Promise<void> {
	const { compile, showCompilerFlags, showVersion, showHelp } = await import(
		"./makensis"
	);
	...
	context.subscriptions.push(
		...
		commands.registerTextEditorCommand(
			"extension.nsis.create-build-task",
			async () => {
		...
		commands.registerTextEditorCommand(
			"extension.nsis.convert-language-file",
			() => {
				return convert();
			},
		),
